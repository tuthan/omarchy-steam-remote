...
| Prior feedback | SteamOS Remote evidence and disposition |
|---|---|
| [Predictable shared temporary files, missing owner/permission/link checks (#146)](https://github.com/omacom/omarchy-plugin-marketplace/issues/146#issuecomment-5292299510) | No shared `/tmp` fallback. Private state lives under `${XDG_STATE_HOME:-~/.local/state}/steamos-remote`. Hardened both credential and pending records with no-follow directory traversal, owner checks, descriptor-relative reads/replacements/deletes, exclusive random temporary files, 0700 directories and 0600 files. Hardlinks and special files are refused. Reads and writes are capped at 64 KiB. |
